...
contract VaquitaPool is Initializable, OwnableUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    event RewardsAdded(uint256 period, uint256 rewardAmount);
    ...
    /**
     * @notice Initializes the contract with the token, access managed MSV, and supported lock periods.
     * @dev Sets up the contract owner, pausable state, and approves the access managed MSV to spend tokens.
     * @param _token The address of the ERC20 token to be deposited.
     * @param _accessManagedMSV The address of the AccessManagedMSV contract.
     * @param _lockPeriods Array of supported lock periods in seconds.
     */
    function initialize(
        address _token,
        address _accessManagedMSV,
        uint256[] calldata _lockPeriods
    ) external initializer {
        __Ownable_init(msg.sender);
        __Pausable_init();
        __ReentrancyGuard_init();
        if (_token == address(0) || _accessManagedMSV == address(0)) revert InvalidAddress();
        token = IERC20(_token);
        accessManagedMSV = IAccessManagedMSV(_accessManagedMSV);
        uint256 length = _lockPeriods.length;
        for (uint256 i = 0; i < length; i++) {
            isSupportedLockPeriod[_lockPeriods[i]] = true;
        }
        token.forceApprove(address(accessManagedMSV), type(uint256).max);
    }
     ...
     * @dev Allows a user to deposit tokens, which are supplied to the AccessManagedMSV. Position is tracked by a unique depositId.
    ...
    function deposit(uint256 amount, uint256 period, uint256 deadline, bytes memory signature) external nonReentrant whenNotPaused returns (uint256 sharesToMint) {
        ...
        // Supply to AccessManagedMSV
        sharesToMint = accessManagedMSV.deposit(amount, address(this));
    ...
    function withdraw(bytes32 depositId) external nonReentrant whenNotPaused returns (uint256 amountToTransfer) {
        ...
        // Withdraw from AccessManagedMSV and get actual amount received
        uint256 withdrawnAmount = accessManagedMSV.redeem(position.shares, address(this), address(this));
     ...
     */
    function _calculateReward(uint256 shares, uint256 period) internal view returns (uint256 reward) {
        uint256 totalSharesForPeriod = periods[period].totalShares;
        if (totalSharesForPeriod == 0) return 0;
        reward = (periods[period].rewardPool * shares) / totalSharesForPeriod;
    }
